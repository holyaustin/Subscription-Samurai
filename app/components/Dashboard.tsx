...
export default function Dashboard() {
  ...
  const stopAgent = async () => {
    try {
      const res = await fetch('/api/agent/start', { method: 'DELETE' });
      const data = await res.json();
      if (data.success) setAgentRunning(false);
    } catch (err) {
              ...
              <div className="flex flex-col gap-2 w-full sm:w-auto">
                {!agentRunning ? (
                  <>
                    <button
                      onClick={startAgent}
                      disabled={starting || noSubscriptions}
                      title={noSubscriptions ? 'Add a subscription first' : 'Start agent'}
                    ...
                    {noSubscriptions && (
                      <p className="text-xs text-amber-600 dark:text-amber-400 text-center">
                        ⚠️ Add a subscription below to enable the agent
                      </p>
                    )}
                  </>
                ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-sm">
              {[
                ...
                <div key={step} className="flex items-start gap-2">
                  ...
                  <span className="text-yellow-700 dark:text-yellow-300">{text}</span>
                </div>
              ))}
            </div>
          </div>
